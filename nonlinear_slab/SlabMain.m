...
%% MESH 
disp('Creating Mesh...');
[nodal_connect, nodal_coordinate, faces, slab_mesh_meta_data , col_mesh_meta_data , col_bot_node, slab_side_nodes,slab_top_nodes,punching_element] = createMesh(slab_dimension,col_dimension,slab_divisions,col_divisions,col_top_postion);
disp('Mesh created');
...
mass_slab = 0;
g = 9.81;
element_density = g*conc_d.*(ones(no_elements,1));
...
% 
% 
% col1_punch = max(max(each_ele_stress(1:4, 4:5, punching_element1)));
% col2_punch = max(max(each_ele_stress(1:4, 4:5, punching_element2)));
% col3_punch = max(max(each_ele_stress(1:4, 4:5, punching_element3)));
% col4_punch = max(max(each_ele_stress(1:4, 4:5, punching_element4)));
%%


% plot(disp1,P, 'DisplayName','500 x500', disp2,P,'DisplayName','550 x550')
% xlabel('Lateral Displacement (mm)')
% ylabel('load applied (N/mm2)')
% legend('show')
function [ nodal_connect, nodal_coordinate, faces, slab_mesh_meta_data , col_mesh_meta_data, col_bot_node, slab_side_nodes,slab_top_nodes  ] = createMesh(slab_dimension,col_dimension,slab_divisions,col_divisions,drop_divisions,col_top_postion,drop_dimension,drop_top_postion )
...
% Gives x_dim, y_dim and z_dim i.e. the mesh size in all the directions.
drop_mesh_size = 50 * floor((drop_dimension./drop_divisions)/50); 

% drop_mesh_size(drop_mesh_size== 0)=50;
% disp(drop_dimension)
% disp(slab_mesh_size)
% disp(col_mesh_size)
% disp(drop_mesh_size)
...
% noc = 4; % No of column
% for i=1:noc
%   `(:,i) = helper(col_mesh_size(1), col_dimension(1),(col_top_postion(i,1) - (col_dimension(1)/2)))';
%   % disp(col_x_coord(:,1))
%   col_y_coord(:,i) = helper(col_mesh_size(2), col_dimension(2),col_top_postion(i,2) - (col_dimension(2)/2))';
%   % disp(col_y_coord(1))
%   col_z_coord(:,i) = helper(col_mesh_size(3), col_dimension(3), 0);
%   col_z_coord_reverse(:,i) = flipud(col_z_coord(:,i)).';
%    % disp(col_z_coord_reverse(:,i))
%   % disp(combvec(col_x_coord(:,1), col_y_coord(:,1), col_z_coord_reverse(:,1)).')
%   col_nodal_coordinate(:,:,i) = combvec(col_x_coord(:,i), col_y_coord(:,i), col_z_coord_reverse(:,i)).';
%   % disp(col_nodal_coordinate(:,:,i));    
%   % disp(combvec(col_x_coord(:,i), col_y_coord(:,i), col_z_coord_reverse(:,i)).')
% end
...
% col_no_elements = 3*2*4;
% col_mesh_meta_data = [3, 2 , 4];
% col_top_index = [8 9 10 11 121 122 123 124 14 15 16 17 181 182 183 184 20 21 22 23  18 18 18 18 101:124 ];
% col_bot_node =[];
% for noc = 1:4
% 	% noc =2;
% for ii = 1:col_no_elements
%     element_no = ii;  
%     layer = floor((element_no-1)/(col_mesh_meta_data(1)*col_mesh_meta_data(2)));
%     % disp(layer)
%     temp = mod(element_no, col_mesh_meta_data(1)*col_mesh_meta_data(2));
%       % disp(temp);
%     if temp
%         element_no = temp;
%         % disp(temp)
%     else
%         element_no = col_mesh_meta_data(1)*col_mesh_meta_data(2);
%    		% disp(element_no)
%     end
%     row = floor((element_no-1)/col_mesh_meta_data(1));
%      % disp(row)
%     temp2 = mod(element_no, col_mesh_meta_data(1));
%     % disp(temp2)
%     if temp2
%         index = temp2;
%     else
%         index = col_mesh_meta_data(1);
%     end
%     if layer == 0
%     	if noc == 1 
%     	node_one = col_top_index(2*row*(col_mesh_meta_data(1)+1) + index);
%     	node_two = max(slab_nodal_connect(:)) + row*(col_mesh_meta_data(1)+1) + index;
%     	 % disp(node_one);
%     	% disp(node_two);
%     	mapping = [node_one , node_one + 1 , col_top_index(2*(row+1)*(col_mesh_meta_data(1)+1) + index +  1) , col_top_index(2*(row+1)*(col_mesh_meta_data(1)+1) + index ) , node_two, node_two + 1,  node_two + col_mesh_meta_data(1) + 2, node_two + col_mesh_meta_data(1) + 1 ];
%     	% disp(mapping);
%     	% face = [mapping(1) mapping(2) mapping(3) mapping(4);
%      %        mapping(5) mapping(6) mapping(7) mapping(8);
%      %        mapping(1) mapping(2) mapping(6) mapping(5);
...
%      %        mapping(2) mapping(3) mapping(7) mapping(6);
%      %        mapping(1) mapping(4) mapping(8) mapping(5)];
%     	% col_nodal_connect(ii, :) = mapping;
%     	% col_faces(6*(ii-1)+1:6*ii, :) = face;
%     	elseif noc == 2
%     		node_one = col_top_index(2*(row)*(col_mesh_meta_data(1)+1) + index + (col_mesh_meta_data(1)+1));
%     		node_two = col1max + row*(col_mesh_meta_data(1)+1) + index;
%     	 %  disp(node_one);
%     	 % disp(node_two);
%     		mapping = [node_one , node_one + 1 , col_top_index(2*(row+1)*(col_mesh_meta_data(1)+1) + index +  1 + (col_mesh_meta_data(1)+1)) , col_top_index(2*(row+1)*(col_mesh_meta_data(1)+1) + index + (col_mesh_meta_data(1)+1)) , node_two, node_two + 1,  node_two + col_mesh_meta_data(1) + 2, node_two + col_mesh_meta_data(1) + 1 ];
%     		% disp(mapping)
%     	elseif noc == 3
%     		node_one = col_top_index(2*(col_mesh_meta_data(2) + 1 )*(col_mesh_meta_data(1) + 1 ) + 2*row*(col_mesh_meta_data(1) + 1 ) + index);
%     		node_two = col2max + row*(col_mesh_meta_data(1)+1) + index;
%     		% disp(node_one)
%     		mapping = [node_one , node_one + 1 , col_top_index(2*(col_mesh_meta_data(1)+1)*(col_mesh_meta_data(2)+1) +  2*(row + 1)*(col_mesh_meta_data(1)+1) + index + 1 ) , col_top_index(2*(col_mesh_meta_data(1)+1)*(col_mesh_meta_data(2)+1) + 2*(row + 1)*(col_mesh_meta_data(1)+1) + index )  , node_two, node_two + 1,  node_two + col_mesh_meta_data(1) + 2, node_two + col_mesh_meta_data(1) + 1 ];
%     		  % disp(mapping)
%     	elseif noc == 4
%     		node_one = col_top_index(2*(col_mesh_meta_data(2) + 1 )*(col_mesh_meta_data(1) + 1 ) + 2*(row)*(col_mesh_meta_data(1)+1) + index + (col_mesh_meta_data(1)+1));
%     		node_two = col3max + row*(col_mesh_meta_data(1)+1) + index;
%     	 %  disp(node_one);
%     	 % disp(node_two);
%     		mapping = [node_one , node_one + 1 , col_top_index(2*(col_mesh_meta_data(2) + 1 )*(col_mesh_meta_data(1) + 1 ) + 2*(row+1)*(col_mesh_meta_data(1)+1) + index +  1 + (col_mesh_meta_data(1)+1)) , col_top_index(2*(col_mesh_meta_data(2) + 1 )*(col_mesh_meta_data(1) + 1 ) + 2*(row+1)*(col_mesh_meta_data(1)+1) + index + (col_mesh_meta_data(1)+1)) , node_two, node_two + 1,  node_two + col_mesh_meta_data(1) + 2, node_two + col_mesh_meta_data(1) + 1 ];
%     		% disp(mapping)
%     	end
%     else
%     	if noc==1
%     		node_one = max(slab_nodal_connect(:)) + (layer-1)*(col_mesh_meta_data(1)+1)*(col_mesh_meta_data(2)+1) + row*(col_mesh_meta_data(1)+1) + index;
%     		node_two = node_one + (col_mesh_meta_data(1)+1)*(col_mesh_meta_data(2)+1);
%     	% disp(node_one)
%     	% disp(node_two)
%     		mapping = [node_one, node_one + 1, node_one + col_mesh_meta_data(1) + 2,  node_one + col_mesh_meta_data(1) + 1 , node_two, node_two + 1,  node_two + col_mesh_meta_data(1) + 2, node_two + col_mesh_meta_data(1) + 1];
%     		col1max = max(mapping(:));
...
%     	elseif noc == 2
%     		node_one = col1max + (layer-1)*(col_mesh_meta_data(1)+1)*(col_mesh_meta_data(2)+1) + row*(col_mesh_meta_data(1)+1) + index;
%     		node_two = node_one + (col_mesh_meta_data(1)+1)*(col_mesh_meta_data(2)+1);
%     	 % disp(node_one)
%     	 % disp(node_two)
%     		mapping = [node_one, node_one + 1, node_one + col_mesh_meta_data(1) + 2,  node_one + col_mesh_meta_data(1) + 1 , node_two, node_two + 1,  node_two + col_mesh_meta_data(1) + 2, node_two + col_mesh_meta_data(1) + 1];
...
%             col_bot_node = unique(col_bot_node);
%         elseif noc == 3
%     		node_one = col2max + (layer-1)*(col_mesh_meta_data(1)+1)*(col_mesh_meta_data(2)+1) + row*(col_mesh_meta_data(1)+1) + index;
%     		node_two = node_one + (col_mesh_meta_data(1)+1)*(col_mesh_meta_data(2)+1);
%     	 % disp(node_one)
%     	 % disp(node_two)
%     		mapping = [node_one, node_one + 1, node_one + col_mesh_meta_data(1) + 2,  node_one + col_mesh_meta_data(1) + 1 , node_two, node_two + 1,  node_two + col_mesh_meta_data(1) + 2, node_two + col_mesh_meta_data(1) + 1];
...
%             col_bot_node = unique(col_bot_node);
%         elseif noc == 4
%     		node_one = col3max + (layer-1)*(col_mesh_meta_data(1)+1)*(col_mesh_meta_data(2)+1) + row*(col_mesh_meta_data(1)+1) + index;
%     		node_two = node_one + (col_mesh_meta_data(1)+1)*(col_mesh_meta_data(2)+1);
%     	 % disp(node_one)
%     	 % disp(node_two)
%     		mapping = [node_one, node_one + 1, node_one + col_mesh_meta_data(1) + 2,  node_one + col_mesh_meta_data(1) + 1 , node_two, node_two + 1,  node_two + col_mesh_meta_data(1) + 2, node_two + col_mesh_meta_data(1) + 1];
...
%             mapping(2) mapping(3) mapping(7) mapping(6);
%             mapping(1) mapping(4) mapping(8) mapping(5)];
%     	col_nodal_connect(ii, :,noc) = mapping;
%     	col_faces(6*(ii-1)+1:6*ii, :,noc) = face;
% end
% end
% disp(length(col_faces(:,:,4)))
% disp(col_nodal_connect(:,:,3))
...
% % slab Helper is made to divide the x coordinate such that it divide according to column when there is column
% function [coordinates,coordx] = slab_helper_x(mesh_size, dimension,coordinates, col_mesh_size , col_dimension,col_top_postion)
%    % disp([mesh_size, dimension,coordinates, col_mesh_size ])
%     coordx = [];
%     flag = 0;
%     flag2 = 0;
%     % disp(mesh_size);	
%     while(flag == 0)
%         coord_val = min(coordinates(end) + mesh_size , coordinates(1) + dimension);
%         % disp(col_top_postion(1,1) - (col_dimension(1)/2));
%          % disp(coord_val)
%         if (coord_val > (col_top_postion(1,1) - (col_dimension(1)/2)) && coord_val <= (col_top_postion(1,1) + (col_dimension(1)/2)))
%         	coord = helper(col_mesh_size(1),col_dimension(1),col_top_postion(1,1)- (col_dimension(1)/2));
%         	coordinates = [coordinates coord];
%         	% disp('hola');
%         	% disp(coord);
%         	coordx = [coordx coord];
%         	flag2 = 1;	
%         elseif (coord_val > (col_top_postion(1,1) + (col_dimension(1)/2)) && min(coordinates(end-1) + mesh_size , coordinates(1) + dimension) <(col_top_postion(1,1) - (col_dimension(1)/2)) )
%         	coord = helper(col_mesh_size(1),col_dimension(1),col_top_postion(1,1)- (col_dimension(1)/2));
%         	coordinates = [coordinates coord];
%         	% disp('hola');
%         	% disp(coord);
%         	coordx = [coordx coord];
%         	flag2 = 1;	
%         elseif (coord_val > (col_top_postion(2,1) - (col_dimension(1)/2)) && coord_val <= (col_top_postion(2,1) + (col_dimension(1)/2)))
%         	coord = helper(col_mesh_size(1),col_dimension(1),col_top_postion(2,1)- (col_dimension(1)/2));
%         	coordinates = [coordinates coord];
%         	flag2 = 1;
%         	coordx = [coordx coord];
%         	% disp('hola');
%         	% disp(coord);
%         elseif (coord_val > (col_top_postion(2,1) + (col_dimension(1)/2)) && min(coordinates(end-1) + mesh_size , coordinates(1) + dimension) < (col_top_postion(2,1) - (col_dimension(1)/2)))
%         	coord = helper(col_mesh_size(1),col_dimension(1),col_top_postion(2,1)- (col_dimension(1)/2));
%         	coordinates = [coordinates coord];
%         	flag2 = 1;
%         	% disp(coord);
...
% % slab Helper is made to divide the y coordinate such that it divide according to column when there is column
% function [coordinates,coordy] = slab_helper_y(mesh_size, dimension,coordinates, col_mesh_size , col_dimension,col_top_postion)
%     coordy = [];
%     flag = 0;
%     flag2 = 0;
%     % disp(mesh_size);	
...
%         % disp(col_top_postion(1,1) - (col_dimension(1)/2));
%          % disp(coord_val)
%         if (coord_val > (col_top_postion(1,2) - (col_dimension(2)/2)) && coord_val <= (col_top_postion(1,2) + (col_dimension(2)/2)))
%         	% disp('hola');
%         	% disp(coord);
%         	flag2 = 1;	
%         	coord = helper(col_mesh_size(2),col_dimension(2),col_top_postion(1,2)- (col_dimension(2)/2));
%         	coordinates = [coordinates coord];
%         	coordy = [coordy coord];
%         	flag2 = 1;	

%         elseif (coord_val > (col_top_postion(1,2) + (col_dimension(2)/2)) && min(coordinates(end-1) + mesh_size , coordinates(1) + dimension) <(col_top_postion(1,2) - (col_dimension(2)/2)) )
%         	coord = helper(col_mesh_size(2),col_dimension(2),col_top_postion(1,2)- (col_dimension(2)/2));
%         	coordinates = [coordinates coord];
%         	% disp('hola');
%         	% disp(coord);
%         	coordy = [coordy coord];
%         	flag2 = 1;	
%         elseif (coord_val > (col_top_postion(3,2) - (col_dimension(2)/2)) && coord_val <= (col_top_postion(3,2) + (col_dimension(2)/2)))
%         	coord = helper(col_mesh_size(2),col_dimension(2),col_top_postion(3,2)- (col_dimension(2)/2));
%         	coordinates = [coordinates coord];
%         	flag2 = 1;
%         	coordy = [coordy coord];
%         	% disp('hola');
%         	% disp(coord);
%         elseif (coord_val > (col_top_postion(3,2) + (col_dimension(2)/2)) && min(coordinates(end-1) + mesh_size , coordinates(1) + dimension) < (col_top_postion(3,2) - (col_dimension(2)/2)))
%         	coord = helper(col_mesh_size(2),col_dimension(2),col_top_postion(3,2)- (col_dimension(2)/2));
%         	coordinates = [coordinates coord];
%         	flag2 = 1;
%         	% disp(coord);

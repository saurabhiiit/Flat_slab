function[element_mapping, col_bot_dof] = ElementMapping(nodal_connect, no_elements, col_bot_node)
%**************************************************************************
...
%                   present in element 1 in counter-clockwise order.
	...
	col_bot_dof =[];
    for j = 1: length(col_bot_node)
    	col_bot_dof = [col_bot_dof 3*col_bot_node(j)-2 3*col_bot_node(j)-1 3*col_bot_node(j)];
    end
